...
class SliderComponent extends HTMLElement {
  constructor() {
    ...
    this.dotElement = this.querySelector(".slider-counter--dot");
  ...
  update() {
    ...
    if (this.currentPageElement && this.pageTotalElement) {
      this.currentPageElement.textContent = this.addLeadingZero(this.currentPage);
      this.pageTotalElement.textContent = this.addLeadingZero(this.sliderItemsToShow.length);

      if (this.dotElement) {
        const currentPagePercentage = this.calculatePercentage(this.currentPage - 1, this.sliderItemsToShow.length);
        const dotElementWidthPercentage = this.calculatePercentage(1, this.sliderItemsToShow.length);

        this.dotElement.style.left = `${currentPagePercentage}%`;
        this.dotElement.style.width = `${dotElementWidthPercentage}%`;
      }
    }
...
class VariantSelects extends HTMLElement {
  ...
  toggleAddButton(disable = true, text, modifyClass = true) {
    ...
    const productStikyForm = document.querySelector(
      `.product-form-${this.dataset.section}-sticky`
    );
    if (!productForm) return;
    const addButton = productForm.querySelector('[name="add"]');
    const addButtonText = productForm.querySelector('[name="add"] > span');
    if (!productStikyForm) return;
    const addButtonSticky = productStikyForm.querySelector('[name="add"]');
    const addButtonStickyText = productStikyForm.querySelector('[name="add"] > span');
    if (!addButton) return;

    let buyButton = document.querySelector('buy-button');
    if (disable) {
      // addButton.setAttribute("disabled", "disabled");
      addButtonSticky.setAttribute("disabled", "disabled");
      if (text) {
        addButtonText.innerHTML = text;
        addButtonStickyText.innerHTML = text;
      }
      addButton.addEventListener('click', buyButton.openModelOutStock , false);
    } else {
      // addButton.removeAttribute("disabled");
      addButtonText.innerHTML = window.variantStrings.addToCart;
      addButtonSticky.removeAttribute("disabled");
      addButtonStickyText.innerHTML = window.variantStrings.addToCart;
      addButton.removeEventListener('click',  buyButton.openModelOutStock, false);
    }
  ...
  setUnavailable() {
    ...
    const buttonSticky = document.querySelector(
      `.product-form-${this.dataset.section}-sticky`
    );
    console.log(buttonSticky)
    ...
    if(!buttonSticky.querySelector('[name="add"]')) {
      buttonSticky.querySelector('[name="add"] > span').textContent = window.variantStrings.unavailable;
    }
  ...
  checkRelatedProduct() {
    this.relatedProducts = document.querySelector('#product-main-related');
    if(this.relatedProducts) {
      this.relatedProductsItem = this.relatedProducts.querySelectorAll('.product-related');
      this.relatedProductsItem.forEach((item) => {
        if(this.getRelatedVariantsData(item.querySelector('[type="application/json"]'))) {
  ...
  getRelatedVariantsData(json) {
    return JSON.parse(json.textContent).some((variant) => variant.title == this.currentVariant.title);
  }
